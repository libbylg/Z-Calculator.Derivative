...
int main(void){
    ...
    while(1){
        ...
        Main->root->child_l = malloc(sizeof(node));
        Main->root->child_l->category = OBJECT;
        Main->root->child_l->type = &Real;
        Main->root->child_l->child_l = NULL;
        ...
        trans_utmost(Main->root->child_l, (void * *)E_utmost_sequence);
